...
class Technology {
    ...
    public function getRandom($branch) {
        ...
        if($type_name == 'Base' || $type_name == 'Preprocessor') {
            ...
            $query = "SELECT TECH.name, TECH.link, IF(TECH.id_ecosystems IS NULL, 'N/A', E.ecosystem) AS ecosystem, T.type AS type, B.branch AS branch FROM $this->table AS TECH
                        INNER JOIN Ecosystems AS E ON TECH.id_ecosystems = E.id OR TECH.id_ecosystems IS NULL
                        INNER JOIN Types AS T ON TECH.id_types = T.id
                        INNER JOIN Branches AS B ON TECH.id_branches = B.id
                        WHERE T.type = :type_name
                        AND B.branch = :branch
